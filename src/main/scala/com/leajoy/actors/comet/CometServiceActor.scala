...
class CometServiceActor(configuration: SiteConfiguration) extends Actor with CometService {
...
class CometActor extends Actor {
  ...
  val rescheduleDuration = 5 seconds  // reschedule time for alive client which hasn't polled since last message
